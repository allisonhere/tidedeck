...
#
# Every checksum must come from the checksums asset the release workflow
# published alongside the tarballs — never from a local `go build`, whose bytes
# are not the ones users download.
set -euo pipefail
